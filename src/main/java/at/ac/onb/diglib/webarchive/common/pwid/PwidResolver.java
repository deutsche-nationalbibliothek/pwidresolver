...
public class PwidResolver {
	...
	public static PWID resolveAny(String archiveString, PwidRegistry registry, Resolver defaultResolver)
			throws PwidParseException, PwidUnsupportedException {
		PWID pwid = null;
		...
		if (!resolver.isSupported(pwid)) {
			throw new PwidUnsupportedException(
					"The requested archive string is unsupported by this resolver: " + archiveString);
		}
		return pwid;
	}
	...
	public String getResolvedUrl(PWID pwid) {
		...
		return UriComponentsBuilder.fromUriString(replay.getBaseUrl())
				.path(renderResolverUriTemplate(replay.getPathPattern(), pwid)).build().toUriString();
	}
	...
	public String getResolvingUri(PWID pwid) {
		...
		return UriComponentsBuilder.fromUriString(resolver.getBaseUrl())
				.path(renderResolverUriTemplate(resolver.getPathPattern(), pwid)).build().toUriString();
	}
