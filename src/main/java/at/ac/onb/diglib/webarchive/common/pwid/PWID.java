 ...
 *
 * Examples:
 * urn:pwid:archive.org:2018-04-10T14:04:11Z:part:https://img1.wsimg.com/isteam/ip/263b8134-2928-4547-b94d-de51b81134fc/311d8d80-cd79-47ee-92bf
 * -99d2cb991f2c.jpg
...
public class PWID {
    ...
    public void setCoverage(PwidCoverage coverage) {
    ...
    public void setTimestamp14(String timestamp14) {
        this.timestamp14 = timestamp14;
    }

    public String getResolvingUri() {
        return resolvingUri;
    }
    ...
    public String toString() {
        return "urn:pwid:" + archiveId + ":" + getDateFormat(WARC_DATE_FORMAT).format(timestamp) + ":"
                + coverage.getValue() + ":" + uri;
    }
    ...
    public static PWID parsePWID(String input) throws PwidParseException {
        ...
        if (!input.startsWith(PWID_PREFIX)) {
            throw new PwidParseException(
                    "The input '" + input + "' is not a valid pwid. It does not begin with 'pwid:'");
        }
        ...
        if (nextColonIndex == -1) {
            throw new PwidParseException(
                    "The input '" + originalInput + "' is not a valid pwid. It does not contain a archiveId");
        }
        ...
        if (nextColonIndex == -1) {
            throw new PwidParseException(
                    "The input '" + originalInput + "' is not a valid pwid. It does not contain a timestamp");
        }
        ...
        if (dateString.isEmpty()) {
            throw new PwidParseException(
                    "The input '" + originalInput + "' is not a valid pwid. It does not contain a valid timestamp");
        }
        Date timestamp = null;
        try {
            timestamp = getDateFormat(WARC_DATE_FORMAT).parse(dateString);
        } catch (ParseException e) {
            throw new PwidParseException("The input '" + originalInput
                    + "' is not a valid pwid. It does not contain a valid timestamp:" + e);
        }
        input = input.substring(nextColonIndex + 1, input.length());
        ...
        if (nextColonIndex == -1) {
            throw new PwidParseException(
                    "The input '" + originalInput + "' is not a valid pwid. It does not contain coverage information");
        }
        String coverage = input.substring(0, nextColonIndex);
        if (coverage.isEmpty()) {
            throw new PwidParseException(
                    "The input '" + originalInput + "' is not a valid pwid. It does not contain a valid coverage");
        }
        PwidCoverage c = null;
        try {
            c = PwidCoverage.valueOf(coverage.toUpperCase());
        } catch (IllegalArgumentException e) {
            throw new PwidParseException("The input '" + originalInput + "' is not a valid pwid. The string '"
                    + coverage + "' is not a valid coverage.");
        }
        input = input.substring(nextColonIndex + 1, input.length());
        String uri = input;
        if (uri.isEmpty()) {
            throw new PwidParseException("The input '" + originalInput + "' is not a valid pwid. The string '" + uri
                    + "' is not a valid Uris.");
        }
